...
describe('raydiumBondSystem', () => {
  ...
  const raydiumTokenAccount = getPubkeyForEnv(
    process.env['RAYDIUM_LP_TOKEN_ACCOUNT']
  )
  ...
  const openOrders = getPubkeyForEnv(process.env['OPEN_ORDERS'])
  ...
  const baseOracle = LUCRA_CONFIG.oracle.SOL_MATA.account
  ...
  it('create raydium bond system', async () => {
    ...
    const settings: BondSystemSettings = {
      ...
      timeclock: BigInt(1),
      epoch: BigInt(1),
      total_amount_to_distribute: BigInt(100000000000),
      max_amount_to_distribute_per_epoch: BigInt(100000000000),
      max_allowed_to_distribute_at_one_time: BigInt(10 * Math.pow(10, 9)),
    }
    ...
    const signature = await sendAndConfirmTransaction(
      connection,
      transaction,
      [creatorKP, ...createBondSystem.signers],
  ...
  it('start bond system', async () => {
    ...
    if (await transactionIsSuccessfull(connection, signatures.create)) {
      ...
      const signature = await sendAndConfirmTransaction(
        connection,
        transaction,
        [creatorKP, ...startBondSystem.signers]
      )
  ...
  it('buy bond', async () => {
    const creatorKP = await createKeypairFromFile(process.env['PAYER_KEYFILE'])
    if (await transactionIsSuccessfull(connection, signatures.start)) {
      waitFor(5 * 1000)
      ...
      await transfer(
        ...
        creatorKP,
        BigInt(10 * Math.pow(10, 9))
      )

      const buyBond = await bondProgram.buyBond(
        'CUSTOM',
        {
          lpTokenAmount: BigInt(1 * Math.pow(10, 9)),
          tokenAccount: raydiumTokenAccount,
      ...
      const signers = [creatorKP, ...buyBond.signers]
  ...
  it('transfer bond', async () => {
    ...
    if (await transactionIsSuccessfull(connection, signatures.buy)) {
      waitFor(5 * 1000)
      ...
      const signature = await sendAndConfirmTransaction(
        connection,
        transaction,
        [creatorKP, ...transferBond.signers]
      )
  ...
  it('exercise bond', async () => {
    ...
    if (await transactionIsSuccessfull(connection, signatures.transfer)) {
      ...
      const signature = await sendAndConfirmTransaction(
        connection,
        transaction,
        [altCreatorKP, ...exerciseBond.signers]
      )
  ...
  it('toggle bond system', async () => {
    const creatorKP = await createKeypairFromFile(process.env['PAYER_KEYFILE'])
    if (await transactionIsSuccessfull(connection, signatures.exercise)) {
      waitFor(5 * 1000)
  ...
  it('close bond system', async () => {
    const creatorKP = await createKeypairFromFile(process.env['PAYER_KEYFILE'])
    if (await transactionIsSuccessfull(connection, signatures.toggle)) {
      waitFor(5 * 1000)
      ...
      const closeBondSystem = await bondProgram.closeBondSystem(
        'CUSTOM',
        {
          ...
          vaultReceiveAccount: raydiumTokenAccount,
        },
        bondSystem
      )
      ...
      const signature = await sendAndConfirmTransaction(
        connection,
        transaction,
        [creatorKP, ...closeBondSystem.signers]
      )
