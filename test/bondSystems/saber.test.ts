...
describe('saberBondSystem', () => {
  ...
  const saberTokenAccount = getPubkeyForEnv(
    process.env['SABER_LP_TOKEN_ACCOUNT']
  )
  ...
  it('create saber bond system', async () => {
    const creatorKP = await createKeypairFromFile(process.env['PAYER_KEYFILE'])

    const settings: BondSystemSettings = {
      pool_type: PoolType.Saber,
      authority: creatorKP.publicKey,
      bond_yield: 9,
      timeclock: BigInt(1),
      epoch: BigInt(1),
      total_amount_to_distribute: BigInt(108999990),
      max_amount_to_distribute_per_epoch: BigInt(108999990),
      max_allowed_to_distribute_at_one_time: BigInt(108999990),
    }
  ...
  it('buy bond', async () => {
    ...
    const buyBond = await bondProgram.buyBond(
      'CUSTOM',
      {
        ...
        tokenAccount: saberTokenAccount,
  ...
  it('close bond system', async () => {
    ...
    const closeBondSystem = await bondProgram.closeBondSystem(
      'CUSTOM',
      {
        ...
        vaultReceiveAccount: saberTokenAccount,
      },
      bondSystem
    )
